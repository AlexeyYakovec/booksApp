   ...
   */
   ...
   class CloneObject {
      ...
      isPathApplicable(changePath) {
         return (
            path.isRootPath(this._path) ||
            path.isSubPath(changePath, this._path)
         );
      }
   }
   ...
   const onChange = (object, onChange, options = {}) => {
      ...
      const isSameObjectTree = (childPath, existingPath) => {
         ...
         const childParts = isArray(childPath)
            ? childPath
            : childPath.split(PATH_SEPARATOR);
         const existingParts = isArray(existingPath)
            ? existingPath
            : existingPath.split(PATH_SEPARATOR);
         ...
         return !existingParts.some(
            (part, index) => part !== childParts[index]
         );
      };
